...
export const useBasketStore = defineStore('basket', () => {
    ...
    function removeItem(id: number) {
        ...
        productStore.excludedProducts = productStore.excludedProducts.filter((product) => product.id !== item?.product.id);
    ...
    function removeBundleItem(basketId: number, bundleProduct: Product) {
        ...
        if (basketItem && basketItem.bundleItems) {
            basketItem.bundleItems = basketItem.bundleItems.filter((item) => item.product.id !== bundleProduct.id);
            productStore.excludedProducts = productStore.excludedProducts.filter((product) => product.id !== bundleProduct.id);
        }
    }
